...
def faceDetection(test_img):
    ...
    face_haar_cascade = cv2.CascadeClassifier('HaarCascade/haarcascade_frontalface_default.xml')  # Load haar
    # classifier
...
def labels_for_training_data2():
    faces = []
    faceID = []

    session = session_factory()
    employeeImage_Dao = EmployeeImage_Dao()
    employee_images = employeeImage_Dao.query_records(session)
    cnt = 1
    if len(employee_images) > 0:
        for images in employee_images:
            # print(cnt)
            # print(cnt, ' : ', images.employee_id)
            nparr = np.fromstring(images.image, np.uint8)
            test_img = cv2.imdecode(nparr, cv2.IMREAD_COLOR)  # cv2.IMREAD_COLOR in OpenCV 3.1
            # test_img = cv2.imread(img_path)  # loading each image one by one
            if test_img is None:
                # print("Image not loaded properly")
                continue
            ...
            roi_gray = gray_img[y:y + w, x:x + h]  # cropping region of interest i.e. face area from grayscale image
            faces.append(roi_gray)
            faceID.append(images.employee_id)
    return faces, faceID
